...
for i in range(0,3):
    for j in range(0,1):
        ...
        #print(i+1,j+1)
        G1,G2,X,Y,E = edge_editing.edge_editing(k,l,i+1,j+1,0,r'..\..\data\as-22july06.gml')
        Y = Y/(1024*1024)
        print(X,Y)
        g.append(G2)
...
sorted_nodes = sorted(mp.items(), key = lambda x: x[1][3])
diff_nodes = {}
for i in range(len(sorted_time)):
    diff_nodes[sorted_nodes[i][0]] = i + 1
sorted_compare = sorted(mp.items(), key = lambda x: x[1][4],reverse= True)
compare_nodes = {}
for i in range(len(sorted_time)):
    compare_nodes[sorted_compare[i][0]] = i + 1
rank = []
for i in title:
    rank.append((i,time[i] + memory[i] + diff_nodes[i] + compare_nodes[i]))
rank = sorted(rank, key = lambda x:x[1])
#print(rank)
print('\n')
print("Number of nodes in internet dataset: ",len(G1.nodes),"Number of edges in internet dataset: ", len(G1.edges))
print('\n')
...
#print(x)
x = x.T
Frame=pd.DataFrame(x.values, columns = ["(Name)","(Time Taken in sec)", "(Memory Consumed in MB)", "(Edge difference)","(Top 20 Centrality)"])
count = 0
for i in range(0,3):
        for j in range(0,5):
