...
for i in range(0,2):
    for j in range(0,1):
        ...
        #print(i+1,j+1)
        G1,G2,X,Y,E = noise_node_addition.noise_node_addition(k,l,i+1,j+1,0,r'..\..\data\as-22july06.gml')
        Y = Y/(1024*1024)
        #print(X,Y)
        g.append(G2)
...
#print(x)
x = x.T
Frame=pd.DataFrame(x.values, columns = ["(Name)","(Time Taken in sec)", "(Memory Consumed in MB)", "(Node difference)","(Top 20 Centrality)"])
count = 0
for i in range(0,3):
        for j in range(0,5):
